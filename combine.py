...
while webcam.isOpened():
    ...
    # Check if there are more men than women and generate a signal if true
    if men_count > women_count and not alert_sent:
        # Send an SMS alert
        responseData = sms.send_message(
            {
                "from": "Vonage APIs",
                "to": "+917871086804",  # Replace with the actual recipient's number
                "text": "Alert: There are more men than women detected in the frame.",
            }
        )
        ...
        # Display the signal on the frame
        cv2.putText(frame, "Signal: More Men", (10, 60), cv2.FONT_HERSHEY_SIMPLEX,
                    0.7, (0, 0, 255), 2)
